...
static void compile_exp(Compiler *c, Exp *e) {
    switch (e->type) {
        ...
        case EXP_TABLE: {
            /* 压入新 table，然后逐条设置条目 */
            emit(c, OP_NEWTABLE, 0, e->line);
            for (int i = 0; i < e->u.table.nentries; i++) {
                TableEntry *en = &e->u.table.entries[i];
                compile_exp(c, en->key);
                compile_exp(c, en->value);
                emit(c, OP_SETINDEX_CONSTR, 0, e->line);
            }
            break;
        }

        case EXP_INDEX: {
            /* 压入 table, key，然后 OP_GETINDEX */
            compile_exp(c, e->u.index.obj);
            compile_exp(c, e->u.index.key);
            emit(c, OP_GETINDEX, 0, e->line);
            break;
        }
    }
}
...
static void compile_stat(Compiler *c, Stmt *s) {
    switch (s->type) {
        ...
        case STMT_INDEX_ASSIGN: {
            /* 压入 table, key, value，然后 OP_SETINDEX */
            compile_exp(c, s->u.index_assign.obj);
            compile_exp(c, s->u.index_assign.key);
            compile_exp(c, s->u.index_assign.value);
            emit(c, OP_SETINDEX, 0, s->line);
            break;
        }
