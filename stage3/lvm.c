        ...
        switch (inst.op) {
            ...
            case OP_LEN: {
                ...
                if (v.type == LUA_TSTRING)
                    ...
                    push(L, make_number((double)table_len(v.v.t)));
                else {
            ...
            /* Table 操作（阶段 3） */
            case OP_NEWTABLE:
                push(L, make_table(table_create()));
                break;

            case OP_GETINDEX: {
                lua_Value key = pop(L);
                lua_Value tbl = pop(L);
                push(L, table_getindex(L, tbl, key, line));
                frame = &L->frames[frame_idx];  /* table_getindex 可能触发 call_function 导致 realloc */
                break;
            }

            case OP_SETINDEX: {
                lua_Value value = pop(L);
                lua_Value key = pop(L);
                lua_Value tbl = pop(L);
                if (tbl.type != LUA_TTABLE) {
                ...
                if (table_get(tbl.v.t, key) != NULL) {
                    ...
                    if (mm && mm->type == LUA_TTABLE) {
                        ...
                        frame = &L->frames[frame_idx];  /* realloc 后重新获取 */
                    } else {
                        table_set(tbl.v.t, key, value);
                    }
                }
            ...
            case OP_SETINDEX_CONSTR: {
                /*
                 * table 构造用：弹出 value, key；栈顶 table[key] = value。
                 * table 不弹出（留在栈顶供后续条目使用）。
                 */
                lua_Value value = pop(L);
                lua_Value key = pop(L);
                lua_Value tbl = L->stack[L->top - 1];  /* peek，不弹出 */
                table_set(tbl.v.t, key, value);
                break;
            }
