/*
 * lobject.h - 核心对象系统：TValue(tagged union) / GCObject / TString / Table / Closure / Proto
 * 官方对照: lua-5.1.5/src/lobject.h
 * 实现阶段: 阶段 2+
 *
 * 阶段 1 简化说明：
 *   官方 lobject.h 定义了完整的 TValue / GCObject / TString / Table / Closure / Proto
 *   体系，配合 GC 和字符串池使用。阶段 1 是树遍历解释器，不需要 GC，因此：
 *   - 值类型用简化的 lua_Value（tagged union，字符串直接用 char*）
 *   - 函数用简化的 Function（内置 C 函数 或 用户函数 + 闭包环境）
 *   - AST 节点（Exp / Stmt）也定义在此文件，因为阶段 1 需要建语法树
...
/*
 * Lua 值的类型标签。
 * 阶段 1 不支持 table（阶段 3 加入），所以没有 LUA_TTABLE。
 */
typedef enum {
    LUA_TNIL = 0,
    LUA_TBOOLEAN,
 ...
 * UpVal — upvalue 运行时表示。
 *
 * open 状态：ptr 指向调用帧的 locals 槽位（外层变量还活着）
 * closed 状态：ptr 指向自身 value（外层已返回，值已复制）
 *
 * 函数返回时 close 所有指向该帧 locals 的 open UpVal：
